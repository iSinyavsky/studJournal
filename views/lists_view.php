...
if(isset($this->select)) { //если выбрали, что-то для отображения, активируем выборку
    ...
    if(isset($_GET['d_error'])){
        switch($this->select){
            case "groups":
                echo '<div style="padding: 5px; background: #fff;">Возникли проблемы с удалением. Отмените все связи с удаляемым элементом.
                <br>Чтобы удалить группу : у группы не должно быть студентов, куратора, старосты.</div>';
            break;
            case "students":
                echo '<div style="padding: 5px; background: #fff;">Возникли проблемы с удалением. Отмените все связи с удаляемым элементом.
                <br>Чтобы удалить студента: студент не должен быть старостой группы.</div>';
            break;
            case "teachers":
                echo '<div style="padding: 5px; background: #fff;">Возникли проблемы с удалением преподавателя. Отмените все связи с удаляемым элементом.
                <br>Чтобы удалить преподавателя: преподаватель не должен быть куратором группы.</div>';
            break;
            case "parents":
                echo '<div style="padding: 5px; background: #fff;">Возникли проблемы с удалением родителя. Отмените все связи с удаляемым элементом.
                <br>Чтобы удалить родителя: родитель не должен иметь детей.</div>';
            break;
    ...
    switch ($this->select) { //что конкретно будем отображать в таблицу
        case "students":
            ...
            foreach ($this->list as $list) {
                if($list->StudFIO != "") {
                    echo '<tr>';
                    echo '<td>' . $list->StudFIO . '</td>';
                    echo '<td>' . $list->GrName . '</td>';
            ...
            break;
        case "groups":
            echo '<th>Группа</th><th>Специальность</th><th>Куратор</th><th>Староста</th>';
            if($role=="admin") echo '<th></th><th></th>';
            foreach ($this->list as $list) {
                    if ($list->GrName != "") {
                        echo '<tr>';
