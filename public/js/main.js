$(document).ready(function () {
    ...
    if(localStorage.getItem("y-scroll")=="") $("#main-panel").scrollTop(0);
    else $("#main-panel").scrollTop(localStorage.getItem("y-scroll"));

    $(".marks").on("scroll",function () {
      localStorage.setItem("x-scroll",$(".marks").scrollLeft());
    });
    $("#main-panel").on("scroll",function(){
        localStorage.setItem("y-scroll",$("#main-panel").scrollTop());
    });
...
function conf(url,list){
    ...
    $("#confirmBox").append("<div ><div class='submit2' style='margin: auto; width: 200px;'>Да</div><div class='submit1' style='margin: auto; width: 200px;'>Нет</div></div>");
...
function warn (text) {
    $("#confirmBox").remove();
    $("body").append("<div id='confirmBox'>"+text+"</div>");
    arr = location.href.split('?');
    $("#confirmBox").append("<div ><a href='"+arr[0]+"'><div class='submit1' style='margin: auto; width: 200px;'>Понял!</div></a></div>");
    $("#confirmBox").slideDown();
    setTimeout(closeM, 6000);
}
